...
const SCALAR_ACTIVE_OWNER_FLIPPED_ENEMY_THIS_TURN: int = 2
...
func restore() -> StateData:
	...
	restored.active_owner_flipped_enemy_this_turn = bool(
		scalars[SCALAR_ACTIVE_OWNER_FLIPPED_ENEMY_THIS_TURN]
	)
...
func _capture_state(state: StateData) -> bool:
	scalars = PackedInt32Array([
		state.active_player,
		state.turn_count,
		int(state.active_owner_flipped_enemy_this_turn),
