...
func _run() -> void:
	...
	_check(selector.debug_get_selected_sect_id() == &"xuanyue_jianzong", "Tapping selects the sect")
	...
	_check(
		...
		== [&"wanyue_guizong", &"qiyao_lianfeng", &"huixue_liuguang", &"hanfeng_liezhen"],
		"Upper hand orders Xuanyue cards from highest tier"
	)
	_check(
	...
	_check(
		selector.debug_get_upper_preview_ids()
		== [&"qianji_tingyu", &"feixing_ruye", &"wusuo_changqiao", &"yuyan_tousuo"],
		"Locked sect previews its highest-tier cards"
	)
	...
	_check(selector.debug_select_sect(&"xuanyue_jianzong"), "Default unlocked sect can be reselected")
	...
	_check(
		String(saved_profile["library_slots"][0]) == "hanfeng_liezhen",
