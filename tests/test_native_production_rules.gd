...
func _test_iterative_attack_lifecycle_matches_recursive() -> void:
	var board: Array = Rules.empty_board()
	var attacker: Dictionary = Catalog.create_instance(
		&"TaiZuChangQuan", Rules.PLAYER_OWNER, &"iterative_attack_source"
	)
	...
	var reacting_target: Dictionary = Catalog.create_instance(
		&"TaiZuChangQuan", Rules.OPPONENT_OWNER, &"iterative_attack_reactor"
	)
	reacting_target["powers"] = [1, 1, 1, 1]
	reacting_target["active_abilities"] = [{
		"triggers": [{
	...
	var protected_target: Dictionary = Catalog.create_instance(
		&"TaiZuChangQuan", Rules.OPPONENT_OWNER, &"iterative_attack_protected"
	)
	protected_target["powers"] = [1, 1, 1, 1]
	protected_target["active_abilities"] = [{
		"triggers": [{
	...
	board[4] = {"owner": Rules.PLAYER_OWNER, "card": attacker}
	board[1] = {"owner": Rules.OPPONENT_OWNER, "card": reacting_target}
	board[3] = {"owner": Rules.OPPONENT_OWNER, "card": protected_target}
	var state := State.new(board, [], [], Rules.PLAYER_OWNER)
	var compact := CompactState.new()
	_check(compact.capture_state(state), "Iterative attack fixture crosses the compact boundary")
	if not compact.is_structurally_valid():
		return
	var kernel: Object = ClassDB.instantiate(&"DuelNativeCompactKernel")
	_check(kernel != null, "Iterative attack fixture creates the native kernel")
	if kernel == null:
		return
	_check(
		bool(kernel.call("load_compact_payload", compact.to_variant_payload())),
		"Iterative attack fixture loads into the native kernel"
	)
	for targeted: bool in [false, true]:
		var request: Dictionary = {
			"source_cell": 4,
			"source_instance_id": &"iterative_attack_source",
			"source_owner_id": Rules.PLAYER_OWNER,
			"mode": &"targeted" if targeted else &"standard",
			"target_cell": 1,
			"target_instance_id": &"iterative_attack_reactor",
			"target_owner_id": Rules.OPPONENT_OWNER,
			"reason": &"test_targeted_attack" if targeted else &"test_standard_attack",
		}
		var recursive: Dictionary = kernel.call(
			"resolve_attack_transition", request
		) as Dictionary
		var iterative: Dictionary = kernel.call(
			"resolve_attack_iterative_for_test", request
		) as Dictionary
		_check(
			iterative == recursive,
			"Explicit attack frame preserves %s attack reactions, flips, and after-attack"
			% ("targeted" if targeted else "standard")
		)


func _test_iterative_attack_actions_match_recursive() -> void:
	...
	for action_index: int in range(attack_actions.size()):
		var board: Array = Rules.empty_board()
		var source_id := StringName("iterative_attack_action_%d" % action_index)
		var source: Dictionary = Catalog.create_instance(
			&"TaiZuChangQuan", Rules.PLAYER_OWNER, source_id
		)
		source["powers"] = [9, 9, 9, 9]
		source["active_abilities"] = [{
			"triggers": [{
				"event": Catalog.TRIGGER_CARD_AFTER_SUMMONED,
				"actions": [attack_actions[action_index]],
			}],
		}]
		var target_id := StringName("iterative_attack_action_target_%d" % action_index)
		var target: Dictionary = Catalog.create_instance(
			&"TaiZuChangQuan", Rules.OPPONENT_OWNER, target_id
		)
		target["powers"] = [1, 1, 1, 1]
		board[4] = {"owner": Rules.PLAYER_OWNER, "card": source}
		board[1] = {"owner": Rules.OPPONENT_OWNER, "card": target}
		var state := State.new(board, [], [], Rules.PLAYER_OWNER)
		var compact := CompactState.new()
		_check(
			compact.capture_state(state),
			"Iterative attack-action fixture crosses the compact boundary (%d)" % action_index
		)
		if not compact.is_structurally_valid():
			continue
		var kernel: Object = ClassDB.instantiate(&"DuelNativeCompactKernel")
		_check(kernel != null, "Iterative attack-action fixture creates the native kernel")
		if kernel == null:
			continue
		_check(
			bool(kernel.call("load_compact_payload", compact.to_variant_payload())),
			"Iterative attack-action fixture loads into the native kernel"
		)
		var context: Dictionary = {
			"trigger_cell": 1,
			"trigger_instance_id": target_id,
			"trigger_owner_id": Rules.OPPONENT_OWNER,
			"trigger_previous_owner_id": Rules.OPPONENT_OWNER,
			"trigger_zone": &"board",
			"trigger_logical_index": 1,
			"trigger_was_on_board": true,
		}
		var recursive: Dictionary = kernel.call(
			"resolve_event_transition", &"card_after_summoned", context
...
func _test_iterative_transfer_resource_matches_recursive() -> void:
	for donor_has_ki: bool in [true, false]:
		var board: Array = Rules.empty_board()
		var source_id := StringName("iterative_transfer_source_%s" % str(donor_has_ki))
		var source: Dictionary = Catalog.create_instance(
			&"TaiZuChangQuan", Rules.PLAYER_OWNER, source_id
		)
		source["active_abilities"] = [{
		...
		var donor: Dictionary = Catalog.create_instance(
			&"TaiZuChangQuan",
			Rules.OPPONENT_OWNER,
			StringName("iterative_transfer_donor_%s" % str(donor_has_ki))
		)
		donor["ki"] = 1 if donor_has_ki else 0
		donor["powers"] = [1, 1, 1, 1]
		donor["active_abilities"] = [{
			"triggers": [{
				"event": Catalog.CARD_BEFORE_EXILED,
				"conditions": [{"type": Catalog.CONDITION_TRIGGER_CARD_IS_SELF}],
				"actions": [{"type": Catalog.ACTION_GAIN_KI, "amount": 1}],
		...
		board[4] = {"owner": Rules.PLAYER_OWNER, "card": source}
		board[0] = {"owner": Rules.OPPONENT_OWNER, "card": donor}
		var state := State.new(board, [], [], Rules.PLAYER_OWNER)
		var compact := CompactState.new()
		_check(
			compact.capture_state(state),
			"Iterative transfer fixture crosses the compact boundary (%s)" % str(donor_has_ki)
		)
		if not compact.is_structurally_valid():
			continue
		var kernel: Object = ClassDB.instantiate(&"DuelNativeCompactKernel")
		_check(kernel != null, "Iterative transfer fixture creates the native kernel")
		if kernel == null:
			continue
		_check(
			bool(kernel.call("load_compact_payload", compact.to_variant_payload())),
			"Iterative transfer fixture loads into the native kernel"
		)
		var context: Dictionary = {
			"trigger_cell": 4,
			"trigger_instance_id": source_id,
		...
		_check(
			iterative == recursive,
			"Resource-transfer frame preserves the %s path"
			% ("ki" if donor_has_ki else "power-with-zero-exile")
		)
