...
func _test_effect_text_formatting() -> void:
	...
	for card_id: StringName in Catalog.get_all_card_ids():
		...
		if description.is_empty():
			continue
		var normalized: String = EffectFormatter.normalize_plain_text(description)
		var paragraphs: Array[String] = EffectFormatter.split_paragraphs(description)
		_check(
			"".join(paragraphs) == normalized.replace("\n", ""),
			"Effect paragraphs preserve every non-layout character for %s" % card_id
		)
		_check(not paragraphs.is_empty(), "Effect paragraphs remain nonempty for %s" % card_id)
...
func _test_trigger_ability_schema() -> void:
	...
	var invalid_abilities: Array[Dictionary] = [
		{},
		{"triggers": []},
		{"triggers": [{"event": &"unknown", "actions": [{"type": Catalog.ACTION_GAIN_KI, "amount": 1}]}]},
		{"triggers": [{"event": Catalog.TRIGGER_END_OWNER_TURN, "actions": []}]},
		{"triggers": [{"event": Catalog.TRIGGER_END_OWNER_TURN, "conditions": {}, "actions": [{"type": Catalog.ACTION_GAIN_KI, "amount": 1}]}]},
		{"triggers": [{"event": Catalog.TRIGGER_END_OWNER_TURN, "conditions": [1], "actions": [{"type": Catalog.ACTION_GAIN_KI, "amount": 1}]}]},
		{"triggers": [{"event": Catalog.TRIGGER_END_OWNER_TURN, "conditions": [{"type": &"unknown"}], "actions": [{"type": Catalog.ACTION_GAIN_KI, "amount": 1}]}]},
