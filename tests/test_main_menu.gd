...
func _run() -> void:
	...
	_check((menu.get_node("MenuLayer/Title") as Label).text == "九宫论剑", "Menu title is exact")
	...
	_check(notice.text == "再按 1 次闭关重修", "Run-reset countdown reaches one")
	run_reset_button.pressed.emit()
	...
	_check(notice.text == "再按 1 次封剑归隐", "Progress-reset countdown reaches one")
	progress_reset_button.pressed.emit()
