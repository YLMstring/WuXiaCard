...
- `test_duel_rules.gd` — shared board geometry/score helpers plus pure native
  attack-target and greedy-query contracts.
...
- `test_duel_replay.gd` — terminal replay gating, inert playback-button
  behavior, supplied icon and touch feedback, exact state reconstruction,
  repeated playback, side-effect suppression, concealment, recovery, and exit.
- `test_duel_undo.gd` — main-deck-gated first-player-decision restoration,
  extra-play checkpoint retention, opponent-response rollback, exact live
  opponent-turn replay, replay truncation, mastery rollback, and fallback from
  unavailable undo.
...
- `test_fumo_qianshou_abilities.gd` — movement-before power loss and stacking,
  ...
  weapon conditions, complete runtime perfect copies, competing original-cell
  summons, and discard-based flip prevention.
...
concealed, use the replay button during live player decisions to verify LaiHe
undo first and exact opponent-turn replay
as fallback, verify the button is inert during full playback while direct card
inspection still pauses a gap, and exit during playback to confirm
original-result routing.
