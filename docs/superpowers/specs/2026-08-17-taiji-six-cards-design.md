...
选择器新增 `CARD_ZONE_REMOVED`，顺序为所属方 `removed_cards` 数组的现有顺序。新增选择条件 `CONDITION_SELECTED_CARD_HAS_NONZERO_POWER`：只要四边中至少一边不为零即合法。
...
三环套月选择时，四边全为零的牌保留在移除区并被跳过；继续寻找下一张合法牌。
...
移除区为空或只有全零牌时，激活仍可选择空位并消耗一内力；选择器产生 `NO_EFFECT`，之后照常抽一张牌。复活实例的完整进场流程结束后才抽牌。
...
5. 三环五阶从任意空格复活同一实例，保留点数/内力/剩余能力，跳过并保留全零牌，成功后从移除区删除。
...
实现前运行完整基线；实现后运行聚焦套件、完整套件，并在正式游戏路径中实际走一次三环复活或乱环连锁流程。
