...
1. 精确来源实例仍能在当前状态中定位；
2. 原能力 handle 仍存在于该实例；
3. 原触发条目仍能从该能力取得；
4. 使用来源实例当前的位置、区域和当前所属方，重新执行声明条件。

不再要求来源处于发现时的区域、格子或所属方，也不再复查
`card_effects_enabled`。所有来源牌移动后都会追踪到新位置继续结算，不再仅对
`trigger_card` 提供移动特例。来源翻面后若精确能力因非保留规则被移除，仍会因
handle 不存在而失效；若能力仍然存在，则以结算时的新所属方判断友方、敌方、
“你”和“对手”。
