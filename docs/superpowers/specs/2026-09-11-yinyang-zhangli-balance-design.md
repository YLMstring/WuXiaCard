...
- 基线源提交：`d95cad0849f8f586076b09f6aee933db3a653a65`；基线原生库
  SHA-256：`DCB3C6DD6F551A6D7551D2ECEE52CE2B463C6C0942F64B926D6731824D867F66`。
- 候选原生库 SHA-256：
  `03B13D51B3E8B1F06FA1F069E2AAC99D59838F71B6584B95F977D90BBCA41293`。
- 同一工作目录、同一台机器、Release 优化的 `template_debug` 原生库、
  Dummy 音频；固定 512 个状态、1,024 个合法动作、3 轮共 3,072 次
  `apply_action`。相邻运行结果为基线 `18.916853s`、候选
  `18.500607s`，候选快约 `2.20%`。结合此前交错样本，未发现可重复的
  3% 以上性能回退，合法行动查询 `1,024/1,024` 一致。
- 阴阳掌力专项测试通过 `43` 项，目录测试通过 `2,159` 项；最终完整
  自动化套件通过：`78` 套，耗时 `265.39s`。
