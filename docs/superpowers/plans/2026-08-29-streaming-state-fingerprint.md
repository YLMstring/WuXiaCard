...
**实施注记：** 纯 GDScript 通用递归版本的微基准只有 legacy 的
`0.543x`，按任务五的停止条件没有进入生产档案。随后保持完整 payload 和
128 位指纹边界，改为 Godot 原生 Variant 二进制编码加 SHA-256/128；该路径
微基准达到 `16.49x`，14 开局生产吞吐提高 `67.05%`，固定深度 oracle
14/14 一致。设计文档的“实施结果”记录了这一有证据支持的实现偏差。
