...
- The later JinYanGong/QiXinJuHui balance pass makes JinYanGong 2–4 draw at
  their current owner's turn end only if that owner has not successfully flipped
  an enemy during that turn. QiXinJuHui 3–4 buff only
...
- Direct rank rewards and the player's chosen reward use a slowly flowing gold
  ...
  active, it replaces the ordinary red/blue owner border and stays within the
  card's original outer bounds, extending slightly inward without changing its
...
- Gated balance telemetry is deployed to the Shanghai CloudBase environment
  `wuxiacard-d9gvg1e2o15e3b73e`. Newly created normal exported runs record only
